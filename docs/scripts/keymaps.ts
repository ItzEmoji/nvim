...
export function renderKeybindingsPage(
  ...
  const header = [
    ...
    '',
    `The leader key is \`${formatLeader(leader)}\`. Press it in the editor and which-key ` +
      'shows you these same descriptions, grouped the same way — that popup is the real ' +
      'reference, and it can never be out of date. This page is the searchable offline ' +
      `copy: all ${keymaps.length} binds, generated from the same Nix evaluation that ` +
      'builds the editor.',
    '',
    '',
  ].join('\n');
